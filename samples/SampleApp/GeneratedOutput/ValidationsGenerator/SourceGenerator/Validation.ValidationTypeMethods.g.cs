...
}public static void Validate(Project value, ref List<ValidationResult> results)
{
    if (value.GetType() != typeof(Project))
    {
        throw new Exception($"Expected to validate Project but got {value.GetType()}");
    }
    var validationContext = new ValidationContext(value);
    validationContext.MemberName = "Project.ProjectUrl";
    validationContext.DisplayName = "Project.ProjectUrl";
    results.Add(Project_ProjectUrl_UrlAttribute.GetValidationResult(value.ProjectUrl, validationContext));
}public static void Validate(IEnumerable<Project> values, ref List<ValidationResult> results)
{
    var validationContext = new ValidationContext(values);
    var index = 0;
    foreach (var value in values)
    {
        validationContext.MemberName = $"Project[{index}].ProjectUrl";
        validationContext.DisplayName = $"Project[{index}].ProjectUrl";
        results.Add(Project_ProjectUrl_UrlAttribute.GetValidationResult(value.ProjectUrl, validationContext));
        index++;
    }
}
