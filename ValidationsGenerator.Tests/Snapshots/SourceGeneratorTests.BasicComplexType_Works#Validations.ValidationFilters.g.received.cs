...
public static partial class Validations
{
    ...
    {
        [("4922e83d-9a83-42de-bc2f-934e5efc584e.cs", 10)] = async (EndpointFilterInvocationContext context, EndpointFilterDelegate next) =>
        {
        ...
        [("4922e83d-9a83-42de-bc2f-934e5efc584e.cs", 13)] = async (EndpointFilterInvocationContext context, EndpointFilterDelegate next) =>
        {
            var results = new List<ValidationResult>();
            var todo = context.GetArgument<Todo>(0);
            Validations.Validate(todo, ref results);
        ...
        [("4922e83d-9a83-42de-bc2f-934e5efc584e.cs", 16)] = async (EndpointFilterInvocationContext context, EndpointFilterDelegate next) =>
        {
            var results = new List<ValidationResult>();
            var todo = context.GetArgument<TodoWithProject>(0);
            Validations.Validate(todo, ref results);
            var errors = new Dictionary<string, string[]>();
            foreach (var result in results)
            {
